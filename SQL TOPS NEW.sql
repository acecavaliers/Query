...
-- @SortBy VARCHAR(100)='{?SortBy}'
 ...
 DECLARE @DATEFROM DATE ='2024-02-01', 
...
SELECT T2.ItmsGrpNam AS 'Department',T3.CANCELED,T1.SWW,'' AS 'DE-AP', '' AS 'TRANSTYPE',
    ...
    T0.NumPerMsr,
    T0.Dscription AS 'Description',
    IIF(T0.NumPerMsr>1,(T0.Quantity-ISNULL(T7.QTY,0))*T0.NumPerMsr,T0.Quantity-ISNULL(T7.QTY,0))AS 'Quantity Sold',
    ...
    IIF(T5.NumPerMsr>1,T5.TTLAPQTY*T5.NumPerMsr,T5.TTLAPQTY)
    -- T5.TTLAPQTY 
     AS 'Quantity Sold',
    ...
    T0.Dscription AS 'Description',
    --T0.Quantity AS 'Quantity Sold',
    IIF(T8.NumPerMsr>1,T8.Quantity*T8.NumPerMsr,T8.Quantity)
    -- T8.Quantity 
        AS 'Quantity Sold',
    ...
    INNER JOIN (SELECT A1.DOCENTRY,REFDOCNUM, A1.QUANTITY,A1.LINETOTAL/A1.QUANTITY AS 'PRICE',A0.IssueDate,NumPerMsr FROM IGE21 A0
            INNER JOIN IGE1 A1 ON A0.DOCENTRY=A1.DOCENTRY AND A1.AcctCode LIKE '%CS010%'
    ...
    T0.NumPerMsr,
    T0.Dscription AS 'Description',
    (IIF(T0.NumPerMsr>1,T0.QUANTITY*T0.NumPerMsr,T0.Quantity)) * -1 AS 'Quantity Sold',
