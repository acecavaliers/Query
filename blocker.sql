	  ...
	  --To ensure that the user should select at least one payment mode and releasing mode for Sales and Back Order (User Defined-Fields)
		   ...
		   BEGIN
			...
			IF exists(SELECT ItemCode FROM ORDR T0 INNER JOIN RDR1 T1	
							ON T0.DocEntry=T1.DocEntry
							 WHERE T0.DocEntry=@list_of_cols_val_tab_del AND U_InvtyStatus='Out of Stocks' AND ([U_BO_Cash]='N' AND T0.[U_BO_CC]= 'N' AND T0.[U_BO_DC]='N'  AND T0.[U_BO_ODC] ='N' AND T0.[U_BO_PDC] ='N'  AND T0.[U_BO_PO]='N' AND T0.[U_BO_OT]='N' AND T0.[U_BO_COD]='N'))
			BEGIN
			...
			BEGIN
					...
					BEGIN
						...
						--BEGIN
								...
								END
								ELSE IF (SELECT COUNT(RefDocNum) FROM RDR21 WHERE DocEntry=@list_of_cols_val_tab_del AND ObjectType=17 AND RefObjType != 14) > 0
								BEGIN
										SET @error=17
...
--
--								IF (SELECT COUNT(RefDocNum) FROM RDR21 WHERE RefDocNum = @iRefDoc AND RefObjType = 22) > 1
--								BEGIN
--										SET @error=19
--										SET @error_message =N'PO in referenced document field already exist.'
--								END
--								ELSE IF (SELECT COUNT(RefDocNum) FROM RDR21 WHERE DocEntry=@list_of_cols_val_tab_del AND ObjectType=17 AND RefObjType != 22) > 0
--								BEGIN
--										SET @error=19
...
--			END
			...
			--To check the existency of SO as referenced document in Sales order from Distribution Center for Drop Ship
			IF  EXISTS(SELECT DocEntry FROM ORDR WHERE DocEntry=@list_of_cols_val_tab_del AND (U_SOType='DC' OR U_SOType='PC'))
			BEGIN
					IF NOT EXISTS (SELECT DocEntry FROM RDR21 WHERE DocEntry=@list_of_cols_val_tab_del AND ObjectType=17 AND RefObjType = 17)
					BEGIN

						SET @error=20
						SET @error_message =N'SO referenced document from Store is required.'
					...
					BEGIN
						...
						BEGIN
								SET @error=20
								SET @error_message =N'Only one SO generated from Store is required in referenced document field.'
						END
						ELSE
						BEGIN
								SELECT @iRefDoc =  T1.RefDocNum FROM ORDR T0 INNER JOIN RDR21 T1 ON T0.DocNum=T1.DocEntry
								...
								IF (SELECT COUNT(RefDocNum) FROM RDR21 WHERE RefDocNum = @iRefDoc AND RefObjType = 17) > 1
								BEGIN
										SET @error=20
										SET @error_message =N'SO referenced document from Store already exist.'
								END
								ELSE IF (SELECT COUNT(RefDocNum) FROM RDR21 WHERE DocEntry=@list_of_cols_val_tab_del AND ObjectType=17 AND (RefObjType != 22 AND RefObjType != 17)) > 0
								BEGIN
										SET @error=20
										SET @error_message =N'Select Sales Order as Transaction Type in the referenced document field.'
								END
								ELSE IF EXISTS(SELECT T0.DocNum FROM ORDR T0 INNER JOIN  RDR21 T1
								ON T0.DocNum = T1.RefDocNum
								WHERE T1.DocEntry=@list_of_cols_val_tab_del AND ObjectType=17 AND RefObjType=17 AND (T0.U_DC IS NULL OR T0.U_DC = ''))
								BEGIN
										SET @error=20
										SET @error_message =N'Sales Order with drop ship warehouse is required in the referenced document field.'
								END

						 END
					END

					IF EXISTS (SELECT DocEntry FROM RDR21 WHERE DocEntry=@list_of_cols_val_tab_del AND ObjectType=17 AND RefObjType = 22)
					BEGIN
							...
							BEGIN
									SET @error=20
									SET @error_message =N'Only one PO is required in referenced document field.'
							END
							ELSE
							BEGIN
									SELECT @iRefDoc =  T1.RefDocNum FROM ORDR T0 INNER JOIN RDR21 T1 ON T0.DocNum=T1.DocEntry
			...
			IF exists (SELECT T0.DocEntry FROM ORDR T0 INNER JOIN RDR1 T1 ON T0.DocNum=T1.DocEntry
			  WHERE T0.DocEntry=@list_of_cols_val_tab_del AND (T0.[U_BO_DRS]='Y' OR T0.U_BO_DSDD='Y' OR T0.U_BO_DSDV='Y' OR T0.U_BO_DSPD='Y') AND (T1.WhsCode NOT LIKE '%DS%'))
			BEGIN
					SET @error=21
			...
			IF exists (SELECT T0.DocEntry FROM ORDR T0 INNER JOIN RDR1 T1 ON T0.DocNum=T1.DocEntry
			  WHERE T0.DocEntry=@list_of_cols_val_tab_del AND (T0.[U_BO_DRS] !='Y' AND T0.U_BO_DSDD !='Y' AND T0.U_BO_DSDV !='Y' AND T0.U_BO_DSPD !='Y') AND (T1.WhsCode LIKE '%DS%'))
			BEGIN
					SET @error=21
			...
			IF EXISTS(SELECT DocNum FROM ORDR T0 INNER JOIN OCRD T1
						  ...
						  ON T0.DocNum=T2.DocEntry INNER JOIN OHEM T3
						  ON T0.OwnerCode=T3.empID					
						  WHERE T0.DocNum=@list_of_cols_val_tab_del AND T2.U_BelowVolDisc='Y' AND T0.U_BelCostAp='N' AND T0.U_PriceApproval='N' AND T3.position = 1 AND T2.BaseRef =''
			...
			ELSE IF EXISTS(SELECT DocNum FROM ORDR T0 INNER JOIN OCRD T1
						  ...
						  AND T2.DiscPrcnt > 0 AND T0.U_PriceApproval='N' AND T0.U_BelCostAp='N' AND T3.position = 1 AND T2.BaseRef =''  AND T2.Dscription NOT LIKE '%DELIVERY CHARGE%' AND T2.WhsCode NOT LIKE '%DS%')
			BEGIN
			...
			BEGIN
				IF EXISTS(SELECT DocNum FROM ORDR T0 INNER JOIN RDR1 T1
								...
								ON T1.ItemCode=T2.ItemCode	INNER JOIN OHEM T3
								ON T0.OwnerCode=T3.empID			
								WHERE T0.DocNum=@list_of_cols_val_tab_del AND T2.WhsCode=T1.WhsCode AND ROUND((T1.PriceAfVAT/1.12),2) < (T2.AvgPrice * T1.NumPerMsr) AND T0.U_BelCostAp='N'
								AND CONVERT(numeric,REPLACE(U_GPBD,',','')) != PriceAfVAT AND T3.position IN (1,3,10) AND T1.BaseRef ='' AND T1.Dscription NOT LIKE '%DELIVERY CHARGE%' AND T1.WhsCode NOT LIKE '%DS%')
				BEGIN
					SET @error=31
					SET @error_message =N'An approval is required for prices below cost. Set the Below Cost Approval field to “Y”.'
				END
			...
			IF (SELECT COUNT(DISTINCT U_Vendor) FROM RDR1 T0 INNER JOIN OWHS T1
				...
				WHERE DocEntry = @list_of_cols_val_tab_del AND T1.DropShip='Y') > 1
			BEGIN
					SET @error=31
			...
			SELECT DISTINCT @iBaseRef = DocEntry FROM RDR1 WHERE DocEntry=@list_of_cols_val_tab_del AND WhsCode LIKE '%DS%'
			...
			IF EXISTS(SELECT DocNum FROM ORDR WHERE DocEntry=@list_of_cols_val_tab_del AND CONVERT(int, (Doctotal % 1) * 100)  NOT IN (25,50,75,0))
		...
		-- AC -- To block Dropship whith uom not eqaul to purchasing uom
		IF  EXISTS (SELECT T0.DocEntry FROM RDR1 T0  INNER JOIN OITM T1 ON T0.ItemCode=T1.ItemCode INNER JOIN OUOM T2 ON T1.BuyUnitMsr=T2.UomName WHERE T0.DocEntry= @list_of_cols_val_tab_del AND WhsCode LIKE '%DS%' AND T2.UomCode <> T0.UomCode )
			BEGIN
				SET @error=34
				SET @error_message =N'Invalid UOM. Use Purchasing UOM.'
			 END

        -- AC -- To block transaction whith same itemcode,uom,freetext 02/29/2024
	...
	--      DECLARE @WarehouseSO VARCHAR(10) = (SELECT DISTINCT WhsCode FROM RDR1 WHERE DocEntry = @list_of_cols_val_tab_del)
	--
	--      IF (@transaction_type='A') AND EXISTS (SELECT DISTINCT T0.WhsCode FROM OWHS T0 WHERE T0.U_CatchUp = 'Y' AND T0.U_CatchUpDate != (SELECT DISTINCT S0.TaxDate FROM ORDR S0 WHERE S0.DocEntry = @list_of_cols_val_tab_del) AND T0.WhsCode = @WarehouseSO)
	--      BEGIN
	--  	    SET @error=35
	...
	--To check if the SO-DTC created by other store for releasing
		IF EXISTS(SELECT DISTINCT T0.DocEntry,LEFT(T2.WhsCode,6),LEFT(T6.Warehouse,6) FROM RDR21 T0 INNER JOIN ORIN T1
					ON T0.RefDocNum=T1.DocNum INNER JOIN RIN1 T2
					ON T1.DocNum=T2.DocEntry INNER JOIN ORDR T3
					ON T0.DocEntry=T3.DocNum INNER JOIN OHEM T4
					ON T3.OwnerCode=T4.empID INNER JOIN OUSR T5
					ON T4.userId=T5.USERID INNER JOIN OUDG T6
					ON T5.DfltsGroup=T6.Code
					WHERE T0.DocEntry=@list_of_cols_val_tab_del AND LEFT(T2.WhsCode,6) <> LEFT(T6.Warehouse,6) AND T0.RefObjType=14)
		BEGIN
				SET @error=51
...
--			IF  EXISTS(SELECT DocEntry FROM OPRQ WHERE DocEntry=@list_of_cols_val_tab_del AND U_POType='TRPBO')
--			BEGIN
...
--						WHERE T0.DocNum=@list_of_cols_val_tab_del AND T1.RefObjType=17 AND (T2.U_BO_PKS='N' AND T2.U_BO_PKO='N' AND T2.U_BO_DS='N' AND T2.U_BO_DO='N'))
--						BEGIN
--								SET @error=19
--								SET @error_message =N'SO mode of releasing in referenced document should be tagged as Back Order (Pick Up - Store/Other Store or Delivery-Store/Other Store).'
--						END
						ELSE
						BEGIN
...
--			END
		...
		IF EXISTS(SELECT T1.DocEntry FROM OPRQ T0 INNER JOIN PRQ1 T1 ON T0.DocNum=T1.DocEntry WHERE T0.DocNum=@list_of_cols_val_tab_del AND T0.CANCELED='N' AND T0.U_POType <> 'TRPDS'  AND T1.WhsCode LIKE '%DS%' AND T1.BaseType=17)
		BEGIN
		...
		--To check if the warehouse used in PR is match with the SO
		IF EXISTS(SELECT T0.DocEntry FROM PRQ1 T0 INNER JOIN RDR1 T1
		ON T0.BaseRef=T1.DocEntry 
		WHERE T0.BaseType=17 AND T0.DocEntry=@list_of_cols_val_tab_del AND RIGHT(T0.WhsCode,2) != RIGHT(T1.WhsCode,2))
		BEGIN
			SET @error=1
...
--bryle--11052022
    ...
    --IF (SELECT U_WSlipSeries From OPOR WHERE DocNUm = @list_of_cols_val_tab_del AND U_ModeOfReleasing = 'Pick_Up' AND U_BO_DRS = 'N' AND U_BO_DSPD = 'N' ) IS NULL
     IF EXISTS (SELECT U_WSlipSeries From OPOR WHERE DocNUm = @list_of_cols_val_tab_del AND DocType = 'I' AND U_ModeOfReleasing = 'Pick_Up' AND U_BO_DRS = 'N' AND U_BO_DSPD = 'N' and U_WSlipSeries IS NULL)
      BEGIN
					SET @error=16
					SET @error_message =N'Update Remarks (WSlip Error).'       	
      END
	...
	--To check if Discount is empty.
	IF EXISTS (SELECT DocnUm FROM OPOR WHERE DOCNUM = @list_of_cols_val_tab_del AND DiscPrcnt IS NULL AND DocType = 'S' )
	BEGIN
			SET @error=18
			SET @error_message =N'Discount % for Document should not be empty.'
	END
...
--BLOCK AND REQUIRE AP WITH ITEM QTY LOWER THAN PO OPNQTY BY ACDC
...
-- BEGIN
--   SET @error = 100
--   SET @error_message = N'AR Invoice No. is required in Referenced Document'
-- END 
...
IF @object_type = '18' AND (@transaction_type='A' OR @transaction_type='U')
	BEGIN 
        ...
        --To check the computation of Withholding Tax base on wtax liable update date 03/07/2024
        SET @fActualWTax = (SELECT ROUND(SUM(WTAmnt),2) FROM PCH5 WHERE AbsEntry=@list_of_cols_val_tab_del)

		SET @fStandardWtax = (SELECT ROUND(SUM(IIF(VatGroup LIKE '%IVT%',GTotal/1.12 ,GTotal))* (SUM(distinct B.Rate)/100),2) from pch1 A 
                            INNER JOIN PCH5 B ON B.AbsEntry=A.DocEntry
                            where DocEntry=@list_of_cols_val_tab_del and WtLiable='y')

		IF @fActualWTax <> @fStandardWtax
		BEGIN
			SET @error=13
			SET @error_message =N'Incorrect withholding tax computation.'+ CONCAT('',@fStandardWtax) 
		END
		...
		BEGIN
                ...
                DECLARE @Apno VARCHAR(20)
                    ...
                    SELECT TOP 1 @Apno=CONCAT('AP ' ,DocEntry) FROM PCH21 WHERE RefDocNum = @iRefDoc AND RefObjType = 13

					--IF (SELECT COUNT(RefDocNum) FROM PCH21 A INNER JOIN OPCH B ON B.DocNum=A.DocEntry AND B.CANCELED='N' WHERE RefDocNum = @iRefDoc AND RefObjType = 13) > 1
					--BEGIN
					--		SET @error=7
					--		SET @error_message =N'AR Invoice referenced document already exist. '+@Apno 
					--END
					--ELSE 
					IF (SELECT COUNT(RefDocNum) FROM PCH21 WHERE DocEntry=@list_of_cols_val_tab_del AND ObjectType=18 AND RefObjType != 13) > 0
					BEGIN
							SET @error=7
    ...
    --To block if store performance is empty-APINV pacomment
...
IF @object_type = '19' AND (@transaction_type='A' OR @transaction_type='U')
	BEGIN 
        ...
        SET @fStandardWtax = (SELECT ROUND(SUM(IIF(VatGroup LIKE '%IVT%',GTotal/1.12 ,GTotal))* (SUM(distinct B.Rate)/100),2) from RPC1 A 
                            INNER JOIN RPC5 B ON B.AbsEntry=A.DocEntry
                            where DocEntry=@list_of_cols_val_tab_del and WtLiable='y')

		IF @fActualWTax <> @fStandardWtax
		BEGIN
			SET @error=7
			SET @error_message =N'Incorrect withholding tax computation.' --+ CONCAT('StandardWtax',@fStandardWtax)
		END
...
IF (@object_type = '13')  AND (@transaction_type='A' OR @transaction_type='U')
	BEGIN 
		...
		IF exists (SELECT T1.[DocNum] FROM OINV T1 INNER JOIN INV1 T2 ON T1.DocEntry=T2.DocEntry
		 WHERE T1.[DocNum]=@list_of_cols_val_tab_del AND T1.DocType='I' AND T1.U_SOType != 'IB' AND (T2.BaseRef='' OR T2.BaseRef IS NULL))
		BEGIN
			SET @error=4
				...
				BEGIN
						...
						BEGIN
							...
							ELSE IF EXISTS(SELECT T0.DocNum,T1.DocEntry,T0.CheckNum,T0.DueDate FROM RCT1 T0 INNER JOIN RCT2  T1
											...
											--WHERE T1.DocEntry=@list_of_cols_val_tab_del AND T1.InvType=13  AND T0.DueDate > CONVERT( varchar, GETDATE(), 101) AND (T0.U_PRNum='' OR T0.U_PRNum IS NULL) AND T0.U_ChkType = 'Post Dated')
	...
	--To check if the mode of releasing is applicable for AR Invoice/AR Reserve Invoice/AR Down Payment Invoice
		...
		SET @vMainWhse = (LEFT((SELECT DISTINCT Warehouse FROM OHEM T0 INNER JOIN OUSR T1 ON T0.userId=T1.USERID INNER JOIN OUDG T2 ON T1.DfltsGroup=T2.Code WHERE T0.empID =  @iOwnerCode),6))
		SET @vExtWhse =  (LEFT((SELECT DISTINCT T3.U_WhseExt FROM OHEM T0 INNER JOIN OUSR T1 ON T0.userId=T1.USERID INNER JOIN OUDG T2 ON T1.DfltsGroup=T2.Code INNER JOIN OWHS T3 ON T2.Warehouse=T3.WhsCode AND T1.DfltsGroup=T2.Code WHERE T0.empID =  @iOwnerCode),6))
		...
		BEGIN
			...
			--Comment 02/27/2024 for special case; Un-comment 02/27/2024 - transaction complete
			IF (NOT EXISTS (SELECT DocEntry FROM  DPI1 WHERE BaseRef = @iBaseRef) AND EXISTS (SELECT DocNum FROM oinv WHERE DocNum=@list_of_cols_val_tab_del AND (U_SO_PO = 'N' AND U_BO_PO ='N' AND U_SO_COD = 'N' AND U_BO_COD ='N' AND U_BO_DSPD='N' AND U_BO_DRS='N'
				AND U_BO_DSDD='N' AND U_BO_DSDV='N')))
			BEGIN
					SET @error=11
			...
			IF EXISTS(SELECT DISTINCT DocNum FROM OINV T0 INNER JOIN INV1 T1 ON T0.DocNum=T1.DocEntry WHERE T0.DocNum=@list_of_cols_val_tab_del AND T0.isIns='N' AND T1.U_PickUpLoc ='WHS' AND T0.CANCELED = 'N' AND (T0.U_SO_PKO = 'Y' OR T0.U_BO_PKO = 'Y'))
			BEGIN
					SET @error=11
		...
		BEGIN
				...
				BEGIN
						...
						SET @error_message =N'User is not allowed to invoice transactions not matched with their default warehouse.'
				END
				IF EXISTS(SELECT DISTINCT DocNum FROM OINV T0 INNER JOIN INV1 T1 ON T0.DocNum=T1.DocEntry WHERE T0.DocNum=@list_of_cols_val_tab_del AND T0.isIns='N' AND T1.U_PickUpLoc ='WHS' AND T0.CANCELED = 'N')
				BEGIN
						SET @error=11
						SET @error_message = N'Use AR Reserve Invoice for pick up to warehouse transactions.'
				END
	...
	--To check if multiple code for store performance exist in one document entry
		...
		IF exists (SELECT DocEntry FROM OINV WHERE DocEntry=@list_of_cols_val_tab_del AND (U_BO_Cash = 'N' AND U_SO_Cash ='N') AND U_TotalCash!='0.00' AND CANCELED='N')
		BEGIN
			SET @error=15
		...
		--For Cash
		IF EXISTS(SELECT DocNum,DocTotal,PaidToDate,U_DocSeries FROM OINV WHERE DocNum=@list_of_cols_val_tab_del AND U_SO_Cash = 'Y' AND U_SO_DTC='N' AND DocTotal > 0 AND CANCELED='N')
		BEGIN
		...
		BEGIN
				...
				IF NOT EXISTS (SELECT DISTINCT DocEntry FROM RDR21 WHERE DocEntry=@iBaseRef AND ObjectType=17 AND RefObjType = 22)
				BEGIN
		...
		--To check if there is SO Type for Distribution Center
		IF exists (SELECT DISTINCT T0.DocEntry FROM OINV T0 INNER JOIN OBPL T1
					ON T0.BPLId=T1.BPLId
					WHERE T0.DocEntry=@list_of_cols_val_tab_del AND T1.U_isDC='Y' AND (T0.U_SOType = '' OR T0.U_SOType IS NULL))
		BEGIN
				SET @error=28
		...
		IF  EXISTS(SELECT DISTINCT T0.DocNum FROM OINV T0 INNER JOIN INV1 T1 ON T0.DocNum=T1.DocEntry
		...
		IF EXISTS(SELECT DISTINCT DocNum FROM OINV T0 INNER JOIN INV1 T1 ON T0.DocNum=T1.DocEntry
					WHERE T0.DocNum=@list_of_cols_val_tab_del AND (T0.U_SOType = 'IB') AND T1.ItemCode NOT IN (SELECT DISTINCT ItemCode FROM PDN1 WHERE DocEntry=T0.U_GRPODocnum))
		BEGIN
				SET @error=30
		...
		END
		 ...
		 --To check the existency of docuement reference in AR Invoice (from GRPO) for DC only
		...
		--To check if the Due Date of PDC exceeds the Payment Terms Due Date
		...
		AND T2.DueDate > T1.DocDueDate)
		BEGIN
			SET @error=7
			SET @error_message =N'Approval for a check date exceeding the documents due date is required. Set the Check Payment Terms Approval field to Y..'
		END
		...
		--To check the existency of PO as referenced document in AR Invoice for Drop Ship Transaction
			IF  EXISTS(SELECT DISTINCT DocEntry FROM INV1 WHERE DocEntry=@list_of_cols_val_tab_del AND WhsCode  LIKE '%DS%')
			BEGIN
					IF NOT EXISTS (SELECT DISTINCT DocEntry FROM INV21 WHERE DocEntry=@list_of_cols_val_tab_del AND ObjectType=13 AND RefObjType = 22)
					BEGIN
					...
					BEGIN
						...
						ELSE
						BEGIN
								SELECT DISTINCT @iRefDoc =  T1.RefDocNum FROM OINV T0 INNER JOIN INV21 T1 ON T0.DocNum=T1.DocEntry
								WHERE T0.DocNum=@list_of_cols_val_tab_del AND T1.RefObjType=22
								...
								BEGIN
										...
										DECLARE @iPRNo1 int, @iPRNo2 int
										SET  @iPRNo1 = (SELECT DISTINCT T0.DocEntry FROM PRQ1 T0 INNER JOIN OPRQ T1 ON T0.DocEntry = T1.DocEntry WHERE T1.Canceled = 'N' AND T0.BASEREF =(SELECT DISTINCT BaseRef FROM INV1 WHERE DocEntry=@list_of_cols_val_tab_del))
    ...
    IF EXISTS (SELECT U_DocSeries FROM OINV WHERE DocEntry = @list_of_cols_val_tab_del AND U_DocSeries NOT LIKE '%CASH%' AND U_DocSeries NOT LIKE '%CHARGE%') 
      BEGIN
        SET @error=50
    ...
    --To Check if AR Invoice is allowed for Catch-Up
      ...
      IF (@transaction_type='A') AND (SELECT DISTINCT CAST(T0.TaxDate  AS DATE) FROM OINV T0 INNER JOIN INV1 T1 ON T0.DocEntry = T1.DocEntry INNER JOIN OWHS T2 ON T1.WhsCode = T2.WhsCode WHERE T2.U_CatchUp = 'N' AND T0.DocEntry = @list_of_cols_val_tab_del AND T2.WhsCode LIKE '%GS') <> CAST(GETDATE() AS DATE)
      BEGIN
  	    SET @error=58
      ...
      IF (@transaction_type='A') AND (SELECT DISTINCT 1 FROM OINV T0 
            INNER JOIN INV1 T1 ON T0.DocEntry = T1.DocEntry 
            INNER JOIN OWHS T2 ON (LEFT(T1.WhsCode,6)+'GS') = (LEFT(T2.WhsCode,6)+'GS') 
            WHERE T2.U_CatchUp = 'Y' 
            AND CONVERT(date,T0.TaxDate)<>CONVERT(date,T2.U_CatchUpDate) 
            AND T0.DocEntry = @list_of_cols_val_tab_del
            AND t2.WhsCode like '%GS'
          ) = 1
      BEGIN
  	    SET @error=52
			  SET @error_message =--(SELECT DISTINCT CONCAT(CONVERT(date,T0.TaxDate),',',CONVERT(date,T2.U_CatchUpDate)) FROM OINV T0 INNER JOIN INV1 T1 ON T0.DocEntry = T1.DocEntry INNER JOIN OWHS T2 ON CONCAT(LEFT(T1.WhsCode,6),'GS') = CONCAT(LEFT(T2.WhsCode,6),'GS') WHERE T2.U_CatchUp = 'Y' AND CONVERT(date,T0.TaxDate)<>CONVERT(date,T2.U_CatchUpDate) AND T0.DocEntry = @list_of_cols_val_tab_del)
              N'Adding a document that falls outside the allowed date is prohibited during encoding.'
      END
	...
	--For Bank Transfer approval for ACCT - direct invoice
		IF EXISTS(SELECT DISTINCT T1.DocNum FROM RCT2 T0 INNER JOIN OINV T1
		ON T0.DocEntry=T1.DocNum INNER JOIN ORCT T2
		ON T0.DocNum=T2.DocNum
		WHERE T0.InvType=13 AND T1.DocNum=@list_of_cols_val_tab_del AND T1.U_BTAppACCT='N'
...
IF (@object_type = '203')  AND (@transaction_type='A' OR @transaction_type='U')
    ...
    IF EXISTS (SELECT U_DocSeries FROM ODPI WHERE DocEntry = @list_of_cols_val_tab_del AND U_DocSeries NOT LIKE '%ARDPI%') 
      ...
      END
		...
		IF exists (SELECT T1.[DocNum] FROM ODPI T1 WHERE T1.[DocNum]=@list_of_cols_val_tab_del
		AND (T1.[U_BO_PO]='Y' OR T1.[U_SO_COD] = 'Y' OR T1.[U_SO_PO]='Y' OR T1.[U_SO_CM]='Y'  OR T1.[U_SO_DTC]='Y' OR T1.[U_BO_COD]='Y' OR T1.[U_SO_HO]='Y' OR T1.[U_BO_HO]='Y') AND DpmPrcnt =100)
		BEGIN
			SET @error=4
			SET @error_message =N'Due to Customer and charge related (except for PDC) transactions are not applicable in AR Down Payment Invoice.'
		END
		...
		BEGIN
			...
			BEGIN
					...
					ELSE IF EXISTS(SELECT T0.DocNum,T1.DocEntry,T0.CheckNum,T0.DueDate FROM RCT1 T0 INNER JOIN RCT2  T1
										...
										--WHERE T1.DocEntry=@list_of_cols_val_tab_del AND T1.InvType=203  AND T0.DueDate > CONVERT( varchar, GETDATE(), 101) AND (T0.U_PRNum='' OR T0.U_PRNum IS NULL) AND T0.U_ChkType='Post Dated')
		...
		BEGIN
			IF NOT EXISTS (SELECT DISTINCT T0.DocNum FROM ORCT T0 INNER JOIN RCT2 T1
			ON T0.DocNum=T1.DocNum INNER JOIN ODPI T2
			ON T1.DocEntry =  T2.DocNum INNER JOIN RCT3 T3
			ON T0.DocNum=T3.DocNum
    ...
    END
      ...
      IF (@transaction_type='A') AND (SELECT DISTINCT CAST(T0.TaxDate  AS DATE) FROM ODPI T0 INNER JOIN DPI1 T1 ON T0.DocEntry = T1.DocEntry INNER JOIN OWHS T2 ON T1.WhsCode = T2.WhsCode WHERE T2.U_CatchUp = 'N' AND T0.DocEntry = @list_of_cols_val_tab_del AND T2.WhsCode LIKE '%GS') <> CAST(GETDATE() AS DATE)
      BEGIN
  	    SET @error=9
			  SET @error_message =N'Adding a document that falls outside the allowed date is prohibited during encoding.'
      END
...
IF @object_type = '14' AND (@transaction_type='A' OR @transaction_type='U')
    ...
    IF EXISTS (SELECT U_DocSeries FROM ORIN WHERE DocEntry = @list_of_cols_val_tab_del AND U_DocSeries NOT LIKE '%CM%') 
      BEGIN
        SET @error=1
      ...
      END
		...
		IF exists (SELECT DocNum FROM ORIN T0 INNER JOIN RIN1 T1 ON T0.DocEntry=T1.DocEntry WHERE T0.DocEntry=@list_of_cols_val_tab_del AND T0.U_ReasonCode=103 AND T1.WhsCode not like '%BS%')
		BEGIN
			SET @error=10
		...
		IF exists (SELECT T0.DocEntry FROM ORIN T0 INNER JOIN RIN1 T1 ON T0.DocEntry=T1.DocEntry
				 WHERE T0.DocEntry=@list_of_cols_val_tab_del AND (U_ActRcv='' OR U_ActRcv IS NULL) AND T1.BaseType <> 203 AND U_ARCMType != 'IB' AND T0.DocType='I')
		BEGIN
			SET @error=11
		...
		IF exists (SELECT T0.DocEntry FROM ORIN T0 INNER JOIN RIN1 T1 ON T0.DocEntry=T1.DocEntry 
					WHERE T0.DocEntry=@list_of_cols_val_tab_del AND  T0.U_ActRcv != T1.WhsCode AND T1.BaseType <> 203 AND DocType='I')
		BEGIN
			SET @error=12
		...
		END
		ELSE IF EXISTS(SELECT T0.DocNum FROM ORIN T0
					INNER JOIN RIN1 T1 ON T0.DocNum=T1.DocEntry
					INNER JOIN OWHS T2 ON T1.WhsCode=T2.WhsCode
		...
		ON T0.BaseRef=T1.DocNum AND T0.BaseType=13 INNER JOIN INV1 T2
		...
		BEGIN
                ...
                -- SET @error_message =(SELECT BaseRef FROM RIN1 WHERE DocEntry=@list_of_cols_val_tab_del)
		END
    ...
    END
      ...
      IF (@transaction_type='A') AND (SELECT DISTINCT CAST(T0.TaxDate  AS DATE) FROM ORIN T0 INNER JOIN RIN1 T1 ON T0.DocEntry = T1.DocEntry INNER JOIN OWHS T2 ON T1.WhsCode = T2.WhsCode WHERE T2.U_CatchUp = 'N' AND T0.DocEntry = @list_of_cols_val_tab_del AND T2.WhsCode LIKE '%GS') <> CAST(GETDATE() AS DATE)
      BEGIN
  	    SET @error=27
			  SET @error_message =N'Adding a document that falls outside the allowed date is prohibited during encoding.'
      END
...
--		END
    ...
    IF (@transaction_type='A') AND EXISTS (SELECT DISTINCT LEFT(T0.WhsCode,6) FROM OWHS T0 WHERE T0.U_CatchUp = 'Y' AND T0.WhsCode LIKE '%GSCNAPBS%' AND T0.U_CatchUpDate != (SELECT DISTINCT S0.TaxDate FROM ODLN S0 WHERE S0.DocEntry = @list_of_cols_val_tab_del) AND LEFT(T0.WhsCode,6) = @ODLNWarehouse)
    BEGIN
  	    SET @error=8
			  SET @error_message =N'Adding a document that falls outside the allowed date is prohibited during catch up encoding.'	
    END

      IF (@transaction_type='A') AND (SELECT DISTINCT CAST(T0.TaxDate  AS DATE) FROM ODLN T0 INNER JOIN DLN1 T1 ON T0.DocEntry = T1.DocEntry INNER JOIN OWHS T2 ON T1.WhsCode = T2.WhsCode WHERE T2.U_CatchUp = 'N' AND T0.DocEntry = @list_of_cols_val_tab_del) <> CAST(GETDATE() AS DATE)
      BEGIN
  	    SET @error=9
			  SET @error_message =N'Adding a document that falls outside the allowed date is prohibited during encoding.'
      END
...
IF @object_type = '20' AND (@transaction_type='A' OR @transaction_type='U')
    ...
    --   (SELECT DISTINCT LEFT(T3.Warehouse,6) FROM OPDN T0 INNER JOIN OHEM T1 ON T0.OwnerCode = T1.empID INNER JOIN OUSR T2 ON T2.INTERNAL_K = T1.userId INNER JOIN OUDG T3 ON T3.Name = T2.USER_CODE  WHERE T0.OwnerCode = @OWNER AND T0.DocEntry = @list_of_cols_val_tab_del)
    --  BEGIN
  	 --   SET @error=21
...
IF @object_type = '24' AND (@transaction_type='A' OR @transaction_type='U')
	BEGIN 
		...
		IF EXISTS(SELECT T0.DocNum FROM OINV T0 INNER JOIN RCT2 T1
		ON T0.DocNum=T1.DocEntry
		WHERE T1.DocNum=@list_of_cols_val_tab_del AND T1.InvType=13 AND T0.U_WTax='Received' AND T1.U_wTaxComCode IS NOT NULL AND T0.U_wTaxComCode IS NOT NULL) 
		BEGIN
				SET @error=40
...
BEGIN 
		...
		BEGIN
			...
			IF EXISTS (SELECT TransId FROM JDT1
						WHERE TransType=30 AND TransId = @list_of_cols_val_tab_del AND TransCode='DPA' AND (Account NOT LIKE 'NA010%' AND Account NOT LIKE 'NA020%' AND Account NOT LIKE 'OP160%' AND Account NOT LIKE 'OP170%'))
			BEGIN
				SET @error=1
...
BEGIN
	...
	--To check the duplication of entry
	SELECT @DOCTYPE=U_Doctype,@DOCNO=U_DocNamber,@msg=(CONCAT(U_Doctype,'. ',U_DocNamber))  FROM  [@REPRINTING] WHERE DocEntry=@list_of_cols_val_tab_del

	IF (SELECT COUNT(DocEntry) FROM [@REPRINTING] WHERE U_Printed='N' AND U_Doctype=@DOCTYPE AND U_DocNamber=@DOCNO)>1
	BEGIN
