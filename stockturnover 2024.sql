...
DECLARE @ITM VARCHAR(50)='{?ItemName}',
...
SELECT DD.ItemCode,TT.ItemName, SUM(invQty) as 'Invoice Total Qty',SUM(invAmt) as 'Invoice Total Amount',SUM(OpenQty) as 'OpenQty',SUM(OpenAmount) as 'OpenAmount',
SUM(ClosingQty) as 'ClosingQty',SUM(ClosingAmount) as 'ClosingAmount'
FROM(
    SELECT ItemCode,SUM(InvoiceTotalQty) as 'invQty',
    ...
    FROM(
        ...
        SELECT ItemCode,Warehouse,OutQty-ISNULL((SELECT SUM(Quantity) FROM RIN1 WHERE BASETYPE =OINM.TransType AND BASEENTRY=OINM.BASE_REF AND ItemCode=OINM.ItemCode),0)
        as 'InvoiceTotalQty'
...
GROUP BY DD.ItemCode,TT.ItemName
