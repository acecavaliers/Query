...
SELECT T2.ItmsGrpNam AS 'Department',T3.CANCELED,T1.SWW,'' AS 'DE-AP', '' AS 'TRANSTYPE',
    ...
    LEFT JOIN (	
                ...
                TA.RefObjType=22
                AND TC.CANCELED='N'
                AND TB.TrgetEntry=0--(SELECT COUNT(TrgetEntry) FROM POR1 WHERE TrgetEntry<>0 AND DocEntry=TC.DocNum)=0
                )AS T5
    ...
    LEFT JOIN (
                ...
                INNER JOIN OPCH TD ON TC.DocEntry=TD.DocNum 
                INNER JOIN POR1 TB ON TB.DocEntry=TC.BaseEntry AND TC.BaseType=22  AND TC.ItemCode=TB.ItemCode
                WHERE 
                TT.RefObjType=13
                AND TD.CANCELED='N'
                ) 
    ...
    INNER JOIN (	
                ...
                INNER JOIN OPOR TC ON TB.DocEntry=TC.DocNum
                INNER JOIN (SELECT AA.BASEREF,AA.BASETYPE, AA.DocEntry,AA.ItemCode,AA.Quantity,AA.ObjType FROM PCH1 AA INNER JOIN OPCH AB ON AA.DocEntry=AB.DocNum WHERE AB.CANCELED='N' AND AA.BaseType=22)
                AS TD ON TD.BASEREF=TB.DOCENTRY AND TD.BASETYPE=TB.ObjType AND TD.ItemCode=TB.ItemCode
