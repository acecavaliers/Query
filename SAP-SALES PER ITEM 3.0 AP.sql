...
-- SET @STORE = REPLACE((@STORE),'ALL STORE','')
 ...
 DECLARE @DATEFROM DATE ='2022-01-01', @DATETO DATE ='2023-05-05',
...
--COST DROPSHIP
LEFT JOIN (	SELECT TB.NumPerMsr,TA.DocEntry,TC.DocNum,TB.BaseEntry,TC.TaxDate ,TB.Quantity,TB.ItemCode,TB.WhsCode,TB.INMPRICE
			,TB.DocEntry AS 'PO_#','PO'AS 'RTYPE'
			,CASE 
			WHEN TC.U_BrokerCode IS NOT NULL AND (SELECT U_VATType from OCRD where CardCode =TC.U_BrokerCode)='Vatable'
			THEN 				
			(TC.U_ARRASTRE+TC.U_BROKERAGE_FEE+TC.U_DOC_STAMP+TC.U_FREIGHT+TC.U_LABOR+TC.U_INSURANCE+TC.U_TRUCKING+TC.U_WHARFAGE+TC.U_OTHERS)/1.12 
...
LEFT JOIN (SELECT TC.NumPerMsr, TT.RefDocNum,TD.DocNum,TD.TaxDate ,TC.Quantity,TC.ItemCode,TC.WhsCode,TC.INMPRICE
		,TC.DocEntry AS 'PO_#','AP'AS 'RTYPE'     
		,CASE 
		WHEN TD.U_BrokerCode IS NOT NULL AND (SELECT U_VATType from OCRD where CardCode =TD.U_BrokerCode)='Vatable'
		THEN 				
		(TD.U_ARRASTRE+TD.U_BROKERAGE_FEE+TD.U_DOC_STAMP+TD.U_FREIGHT+TD.U_LABOR+TD.U_INSURANCE+TD.U_TRUCKING+TD.U_WHARFAGE+TD.U_OTHERS)/1.12 
        ...
        WHERE 
        TT.RefObjType=13
        AND TD.CANCELED='N')AS T8
		ON T8.RefDocNum=T0.BASEENTRY AND T8.ItemCode=T0.ItemCode
--END COST DROP SHIP
...
AND D.CANCELED='N'
-- AND Transaction# IN (100,170,192) 
-- and [TYPE]='AR Credit Memo'
