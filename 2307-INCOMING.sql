...
DECLARE @QTR INT= {?quarter} ,@GRP INT ={?Group}, @ATC varchar(9)= '{?ATC}' , @INVnum  INT = {?ApDocNum}, @INV Int ={?InvType}, @DOCNUM INT ={?DocNum}
...
CASE WHEN 
    CASE WHEN T0.DOCTYPE = 'I' THEN (Select TOP 1 Left(FedTaxID,3) FROM OWHS WHERE WhsCode=(SELECT TOP 1 WhsCode FROM INV1 WHERE DocEntry=T0.DocNum))
    WHEN T0.DOCTYPE = 'S' THEN (Select TOP 1 Left(FedTaxID,3) FROM OWHS WHERE WhsCode=(SELECT TOP 1 OOCR.U_Whse FROM INV1 
                                INNER JOIN OOCR ON INV1.OCRCODE = OOCR.OCRCODE WHERE DocEntry=T0.DocNum)) END IS NULL 
    ...
    CASE WHEN T0.DOCTYPE = 'I' THEN (Select TOP 1 Left(FedTaxID,3) FROM OWHS WHERE WhsCode=(SELECT TOP 1 WhsCode FROM INV1 WHERE DocEntry=T0.DocNum))
    WHEN T0.DOCTYPE = 'S' THEN (Select TOP 1 Left(FedTaxID,3) FROM OWHS WHERE WhsCode=(SELECT TOP 1 OOCR.U_Whse FROM INV1 
                                 INNER JOIN OOCR ON INV1.OCRCODE = OOCR.OCRCODE WHERE DocEntry=T0.DocNum)) END
END AS 'PY1stTIN',

CASE WHEN 
    CASE WHEN T0.DOCTYPE = 'I' THEN (Select TOP 1 SUBSTRING(FedTaxID,5,3) FROM OWHS WHERE WhsCode=(SELECT TOP 1 WhsCode FROM INV1 WHERE DocEntry=T0.DocNum))
    WHEN T0.DOCTYPE = 'S' THEN (Select TOP 1 SUBSTRING(FedTaxId,5,3) FROM OWHS WHERE WhsCode=(SELECT TOP 1 OOCR.U_Whse FROM INV1 
                                INNER JOIN OOCR ON INV1.OCRCODE = OOCR.OCRCODE WHERE DocEntry=T0.DocNum)) END IS NULL 
...
CASE WHEN 
    CASE WHEN T0.DOCTYPE = 'I' THEN  (Select TOP 1 SUBSTRING(FedTaxID,9,3) FROM OWHS WHERE WhsCode=(SELECT TOP 1 WhsCode FROM INV1 WHERE DocEntry=T0.DocNum))
    WHEN T0.DOCTYPE = 'S' THEN (Select TOP 1 SUBSTRING(FedTaxId,9,3) FROM OWHS WHERE WhsCode=(SELECT TOP 1 OOCR.U_Whse FROM INV1 
                                INNER JOIN OOCR ON INV1.OCRCODE = OOCR.OCRCODE WHERE DocEntry=T0.DocNum)) END IS NULL 
...
CASE WHEN 
    ...
    (Select TOP 1 SUBSTRING(FedTaxID,13,3) FROM OWHS WHERE WhsCode=(SELECT TOP 1 WhsCode FROM INV1 WHERE DocEntry=T0.DocNum))
    WHEN T0.DOCTYPE = 'S' THEN
    (Select TOP 1 SUBSTRING(FedTaxId,13,3) FROM OWHS WHERE WhsCode=(SELECT TOP 1 OOCR.U_Whse FROM INV1 
    INNER JOIN OOCR ON INV1.OCRCODE = OOCR.OCRCODE WHERE DocEntry=T0.DocNum)) END IS NULL THEN 
...
CASE WHEN 
    ...
    City,(CASE WHEN City IS NULL THEN '' ELSE ' ' END)) FROM owhs  WHERE WhsCode=(SELECT TOP 1 WhsCode FROM INV1 WHERE DocEntry=T0.DocNum) )) 
    WHEN T0.DOCTYPE = 'S' THEN
    UPPER((SELECT TOP 1 CONCAT(CAST(Building AS varchar(max)),(CASE WHEN Building IS NULL THEN '' ELSE ', ' END),
    Street,(CASE WHEN Street IS NULL THEN '' ELSE ', ' END),StreetNo,(CASE WHEN StreetNo IS NULL THEN '' ELSE ', ' END),[Block],(CASE WHEN [Block] IS NULL THEN '' ELSE ', ' END),
    City,(CASE WHEN City IS NULL THEN '' ELSE ' ' END)) FROM owhs  WHERE WhsCode=(
    SELECT TOP 1 OOCR.U_Whse FROM INV1 
    INNER JOIN OOCR ON INV1.OCRCODE = OOCR.OCRCODE WHERE DocEntry=T0.DocNum) )) END IS NULL 
    THEN
...
CASE WHEN 
    CASE WHEN T0.DOCTYPE = 'I' THEN (Select TOP 1 Left(FedTaxID,3) FROM OWHS WHERE WhsCode=(SELECT TOP 1 WhsCode FROM DPI1 WHERE DocEntry=T0.DocNum))
    WHEN T0.DOCTYPE = 'S' THEN (Select TOP 1 Left(FedTaxID,3) FROM OWHS WHERE WhsCode=(SELECT TOP 1 OOCR.U_Whse FROM DPI1 
                                INNER JOIN OOCR ON DPI1.OCRCODE = OOCR.OCRCODE WHERE DocEntry=T0.DocNum)) END IS NULL 
    THEN (SELECT  TOP 1 Left(FedTaxID,3)
            FROM  OWHS A0 
            inner join oudg  A1 on A0.WhsCode=A1.Warehouse
            inner join OUSR A2 on A1.Code=A2.DfltsGroup
    ...
    CASE WHEN T0.DOCTYPE = 'I' THEN (Select TOP 1 Left(FedTaxID,3) FROM OWHS WHERE WhsCode=(SELECT TOP 1 WhsCode FROM DPI1 WHERE DocEntry=T0.DocNum))
    WHEN T0.DOCTYPE = 'S' THEN (Select TOP 1 Left(FedTaxID,3) FROM OWHS WHERE WhsCode=(SELECT TOP 1 OOCR.U_Whse FROM DPI1 
                                 INNER JOIN OOCR ON DPI1.OCRCODE = OOCR.OCRCODE WHERE DocEntry=T0.DocNum)) END
END AS 'PY1stTIN',

CASE WHEN 
    CASE WHEN T0.DOCTYPE = 'I' THEN (Select TOP 1 SUBSTRING(FedTaxID,5,3) FROM OWHS WHERE WhsCode=(SELECT TOP 1 WhsCode FROM DPI1 WHERE DocEntry=T0.DocNum))
    WHEN T0.DOCTYPE = 'S' THEN (Select TOP 1 SUBSTRING(FedTaxId,5,3) FROM OWHS WHERE WhsCode=(SELECT TOP 1 OOCR.U_Whse FROM DPI1 
                                INNER JOIN OOCR ON DPI1.OCRCODE = OOCR.OCRCODE WHERE DocEntry=T0.DocNum)) END IS NULL 
    THEN (SELECT  TOP 1 SUBSTRING(FedTaxID,5,3)
        FROM  OWHS A0 
        inner join oudg  A1 on A0.WhsCode=A1.Warehouse
...
CASE WHEN 
    CASE WHEN T0.DOCTYPE = 'I' THEN  (Select TOP 1 SUBSTRING(FedTaxID,9,3) FROM OWHS WHERE WhsCode=(SELECT TOP 1 WhsCode FROM DPI1 WHERE DocEntry=T0.DocNum))
    WHEN T0.DOCTYPE = 'S' THEN (Select TOP 1 SUBSTRING(FedTaxId,9,3) FROM OWHS WHERE WhsCode=(SELECT TOP 1 OOCR.U_Whse FROM DPI1 
                                INNER JOIN OOCR ON DPI1.OCRCODE = OOCR.OCRCODE WHERE DocEntry=T0.DocNum)) END IS NULL 
    THEN (SELECT  TOP 1 SUBSTRING(FedTaxID,9,3)
            FROM  OWHS A0 
            inner join oudg  A1 on A0.WhsCode=A1.Warehouse
...
CASE WHEN 
    CASE WHEN T0.DOCTYPE = 'I' THEN
    (Select TOP 1 SUBSTRING(FedTaxID,13,3) FROM OWHS WHERE WhsCode=(SELECT TOP 1 WhsCode FROM DPI1 WHERE DocEntry=T0.DocNum))
    WHEN T0.DOCTYPE = 'S' THEN
    (Select TOP 1 SUBSTRING(FedTaxId,13,3) FROM OWHS WHERE WhsCode=(SELECT TOP 1 OOCR.U_Whse FROM DPI1 
    INNER JOIN OOCR ON DPI1.OCRCODE = OOCR.OCRCODE WHERE DocEntry=T0.DocNum)) END IS NULL THEN 
    (SELECT  TOP 1 SUBSTRING(FedTaxID,13,3)
    FROM  OWHS A0 
    inner join oudg  A1 on A0.WhsCode=A1.Warehouse
    inner join OUSR A2 on A1.Code=A2.DfltsGroup
...
CASE WHEN 
    ...
    UPPER((SELECT TOP 1 CONCAT(CAST(Building AS varchar(max)),(CASE WHEN Building IS NULL THEN '' ELSE ', ' END),
    Street,(CASE WHEN Street IS NULL THEN '' ELSE ', ' END),StreetNo,(CASE WHEN StreetNo IS NULL THEN '' ELSE ', ' END),[Block],(CASE WHEN [Block] IS NULL THEN '' ELSE ', ' END),
    City,(CASE WHEN City IS NULL THEN '' ELSE ' ' END)) FROM owhs  WHERE WhsCode=(SELECT TOP 1 WhsCode FROM DPI1 WHERE DocEntry=T0.DocNum) )) 
    WHEN T0.DOCTYPE = 'S' THEN
    UPPER((SELECT TOP 1 CONCAT(CAST(Building AS varchar(max)),(CASE WHEN Building IS NULL THEN '' ELSE ', ' END),
    Street,(CASE WHEN Street IS NULL THEN '' ELSE ', ' END),StreetNo,(CASE WHEN StreetNo IS NULL THEN '' ELSE ', ' END),[Block],(CASE WHEN [Block] IS NULL THEN '' ELSE ', ' END),
    City,(CASE WHEN City IS NULL THEN '' ELSE ' ' END)) FROM owhs  WHERE WhsCode=(
    SELECT TOP 1 OOCR.U_Whse FROM DPI1 
    INNER JOIN OOCR ON DPI1.OCRCODE = OOCR.OCRCODE WHERE DocEntry=T0.DocNum) )) END IS NULL 
    THEN
...
CASE WHEN 
    CASE WHEN T0.DOCTYPE = 'I' THEN (Select TOP 1 Left(FedTaxID,3) FROM OWHS WHERE WhsCode=(SELECT TOP 1 WhsCode FROM RIN1 WHERE DocEntry=T0.DocNum))
    WHEN T0.DOCTYPE = 'S' THEN (Select TOP 1 Left(FedTaxID,3) FROM OWHS WHERE WhsCode=(SELECT TOP 1 OOCR.U_Whse FROM RIN1 
                                INNER JOIN OOCR ON RIN1.OCRCODE = OOCR.OCRCODE WHERE DocEntry=T0.DocNum)) END IS NULL 
    THEN (SELECT  TOP 1 Left(FedTaxID,3)
            FROM  OWHS A0 
            inner join oudg  A1 on A0.WhsCode=A1.Warehouse
            inner join OUSR A2 on A1.Code=A2.DfltsGroup
    ...
    CASE WHEN T0.DOCTYPE = 'I' THEN (Select TOP 1 Left(FedTaxID,3) FROM OWHS WHERE WhsCode=(SELECT TOP 1 WhsCode FROM RIN1 WHERE DocEntry=T0.DocNum))
    WHEN T0.DOCTYPE = 'S' THEN (Select TOP 1 Left(FedTaxID,3) FROM OWHS WHERE WhsCode=(SELECT TOP 1 OOCR.U_Whse FROM RIN1 
                                 INNER JOIN OOCR ON RIN1.OCRCODE = OOCR.OCRCODE WHERE DocEntry=T0.DocNum)) END
END AS 'PY1stTIN',

CASE WHEN 
    CASE WHEN T0.DOCTYPE = 'I' THEN (Select TOP 1 SUBSTRING(FedTaxID,5,3) FROM OWHS WHERE WhsCode=(SELECT TOP 1 WhsCode FROM RIN1 WHERE DocEntry=T0.DocNum))
    WHEN T0.DOCTYPE = 'S' THEN (Select TOP 1 SUBSTRING(FedTaxId,5,3) FROM OWHS WHERE WhsCode=(SELECT TOP 1 OOCR.U_Whse FROM RIN1 
                                INNER JOIN OOCR ON RIN1.OCRCODE = OOCR.OCRCODE WHERE DocEntry=T0.DocNum)) END IS NULL 
    THEN (SELECT  TOP 1 SUBSTRING(FedTaxID,5,3)
        FROM  OWHS A0 
        inner join oudg  A1 on A0.WhsCode=A1.Warehouse
...
CASE WHEN 
    CASE WHEN T0.DOCTYPE = 'I' THEN  (Select TOP 1 SUBSTRING(FedTaxID,9,3) FROM OWHS WHERE WhsCode=(SELECT TOP 1 WhsCode FROM RIN1 WHERE DocEntry=T0.DocNum))
    WHEN T0.DOCTYPE = 'S' THEN (Select TOP 1 SUBSTRING(FedTaxId,9,3) FROM OWHS WHERE WhsCode=(SELECT TOP 1 OOCR.U_Whse FROM RIN1 
                                INNER JOIN OOCR ON RIN1.OCRCODE = OOCR.OCRCODE WHERE DocEntry=T0.DocNum)) END IS NULL 
    THEN (SELECT  TOP 1 SUBSTRING(FedTaxID,9,3)
            FROM  OWHS A0 
            inner join oudg  A1 on A0.WhsCode=A1.Warehouse
...
CASE WHEN 
    CASE WHEN T0.DOCTYPE = 'I' THEN
    (Select TOP 1 SUBSTRING(FedTaxID,13,3) FROM OWHS WHERE WhsCode=(SELECT TOP 1 WhsCode FROM RIN1 WHERE DocEntry=T0.DocNum))
    WHEN T0.DOCTYPE = 'S' THEN
    (Select TOP 1 SUBSTRING(FedTaxId,13,3) FROM OWHS WHERE WhsCode=(SELECT TOP 1 OOCR.U_Whse FROM RIN1 
    INNER JOIN OOCR ON RIN1.OCRCODE = OOCR.OCRCODE WHERE DocEntry=T0.DocNum)) END IS NULL THEN 
    (SELECT  TOP 1 SUBSTRING(FedTaxID,13,3)
    FROM  OWHS A0 
    inner join oudg  A1 on A0.WhsCode=A1.Warehouse
    inner join OUSR A2 on A1.Code=A2.DfltsGroup
...
CASE WHEN 
    ...
    UPPER((SELECT TOP 1 CONCAT(CAST(Building AS varchar(max)),(CASE WHEN Building IS NULL THEN '' ELSE ', ' END),
    Street,(CASE WHEN Street IS NULL THEN '' ELSE ', ' END),StreetNo,(CASE WHEN StreetNo IS NULL THEN '' ELSE ', ' END),[Block],(CASE WHEN [Block] IS NULL THEN '' ELSE ', ' END),
    City,(CASE WHEN City IS NULL THEN '' ELSE ' ' END)) FROM owhs  WHERE WhsCode=(SELECT TOP 1 WhsCode FROM RIN1 WHERE DocEntry=T0.DocNum) )) 
    WHEN T0.DOCTYPE = 'S' THEN
    UPPER((SELECT TOP 1 CONCAT(CAST(Building AS varchar(max)),(CASE WHEN Building IS NULL THEN '' ELSE ', ' END),
    Street,(CASE WHEN Street IS NULL THEN '' ELSE ', ' END),StreetNo,(CASE WHEN StreetNo IS NULL THEN '' ELSE ', ' END),[Block],(CASE WHEN [Block] IS NULL THEN '' ELSE ', ' END),
    City,(CASE WHEN City IS NULL THEN '' ELSE ' ' END)) FROM owhs  WHERE WhsCode=(
    SELECT TOP 1 OOCR.U_Whse FROM RIN1 
    INNER JOIN OOCR ON RIN1.OCRCODE = OOCR.OCRCODE WHERE DocEntry=T0.DocNum) )) END IS NULL 
    THEN
