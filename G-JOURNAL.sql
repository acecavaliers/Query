...
INNER  JOIN (SELECT DocEntry ,ObjCode, 'AR W/ARDPI' AS 'TRANS_TP' FROM  INV9 
            ...
            UNION ALL
            SELECT DocNum AS 'DocEntry',ObjType, 'INCOMING CHECK' AS 'TRANS_TP' FROM ORCT where CheckAcct is not null 
            UNION ALL
            SELECT DocNum AS 'DocEntry',ObjType, 'INCOMING CARD' AS 'TRANS_TP' FROM ORCT where CheckAcct is  null and CashAcct is null and TrsfrAcct is null
            UNION ALL 
            ...
            WHERE CANCELED='N' --AND CardCode IN (SELECT CARDCODE FROM OCRD WHERE CARDTYPE='S' AND U_TaxPayerClass='Y' AND Affiliate='Y' )
