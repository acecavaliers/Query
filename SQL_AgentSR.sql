...
set @Todate = '2024-02-20'

set @Trans='Customer'

set @WHS= replace('atp', 'All','')
...
BEGIN
    ...
    SELECT C, CardCode,CardName,[month],GRSSmonth,SUM(Volume)/1.12 AS TTL,SUM(GRSS) AS GRSS FROM (
        SELECT DISTINCT T2.CardCode,t2.CardName, T3.Name AS C, T2.DocTotal Volume, 
        ...
        FROM ODPI T2
        INNER JOIN dbo.[@SALESAGENT] T3 ON T3.Code=T2.U_SalesAgent 
        INNER JOIN DPI1 T4 ON T4.DOCENTRY=T2.DocNum
        WHERE TaxDate between @fromdate and @todate
        AND U_SALESAGENT<>''
        AND CANCELED='N'
        AND REPLACE(WhsCode,'KORKM2','KOROST') LIKE '%'+@WHS+'%'
        ...
        SELECT DISTINCT T2.CardCode,t2.CardName, T3.Name AS C, t2.DocTotal*-1 as Volume, 
...
BEGIN
    ...
    SELECT C, ItemCode,[month],GRSSmonth,SUM(Volume)/1.12  AS TTL,SUM(GRSS) AS GRSS FROM (
