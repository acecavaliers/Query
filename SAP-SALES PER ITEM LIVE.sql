...
SELECT T2.ItmsGrpNam AS 'Department',T3.CANCELED,T1.SWW,'' AS 'DE-AP','' AS 'TRANSTYPE',
T3.BPLID AS 'BRANCH ID',
CASE WHEN T0.DOCENTRY IN (SELECT A0.BaseEntry FROM DLN1 A0 WHERE A0.ITEMCODE=T0.ITEMCODE AND A0.BaseEntry=T0.DOCENTRY ) 
THEN 'AR RESERVE-WITH DELIVERY' ELSE 'AR RESERVE-NO DELIVERY' END AS 'TYPE',
T1.U_Category AS 'Category',
T0.DocEntry AS 'Transaction#',T6.Number,
T3.DocDate AS 'Posting Date',
t3.U_DocSeries AS 'Invoice No.',
...
CASE WHEN T6.RTYPE='AP' 
THEN T6.DocEntry
WHEN T7.RTYPE='AP' THEN T7.DOCENTRY
WHEN T8.RTYPE='AP' THEN T8.DocEntry
WHEN T9.DocEntry IS NOT NULL THEN T9.DocEntry
END AS 'DE-AP',
CASE WHEN ISNULL(ISNULL(T6.INMPrice,ISNULL(T7.INMPrice,T8.INMPrice)),T9.INMPrice) IS NOT NULL THEN 18 ELSE 0 END AS 'TRANSTYPE',
T3.BPLID AS 'BRANCH ID',
...
T3.DocDate AS 'Posting Date',
t3.U_DocSeries AS 'Invoice No.',
CASE WHEN  T6.DocEntry IS NOT NULL AND T6.RTYPE ='AP'
THEN CONCAT('AP ',T6.DocEntry)
WHEN T7.RTYPE='AP' AND T7.RefDocNum IS NOT NULL
THEN CONCAT('AP ',T7.REFDOCNUM)
WHEN T9.DocEntry IS NOT NULL 
THEN CONCAT('AP ',T9.DocEntry)
WHEN T8.DocEntry IS NOT NULL AND T8.RTYPE='AP'
THEN CONCAT('AP ',T8.DocEntry)
ELSE ''
END AS 'Reference',
CASE WHEN T6.RTYPE ='AP' AND T6.TAXDATE IS NOT NULL
THEN CONVERT(VARCHAR(20),T6.TAXDATE,101)
WHEN T7.RTYPE='AP' AND T7.ISSUEDATE IS NOT NULL
THEN CONVERT(VARCHAR(20),T7.ISSUEDATE,101)
WHEN T9.TaxDate IS NOT NULL 
THEN CONVERT(VARCHAR(20),T9.TAXDATE,101)
WHEN T8.IssueDate IS NOT NULL AND T8.RTYPE='AP'
...
T0.Dscription AS 'Description',
CASE WHEN 
(SELECT COUNT(Quantity)FROM POR1 WHERE DocEntry=T6.DocEntry AND ItemCode=T6.ItemCode AND ObjType=T6.ObjType 
AND DOCENTRY NOT IN (SELECT BASEENTRY FROM PCH1 WHERE BaseType=22 AND ITEMCODE IS NOT NULL)) +
(SELECT COUNT(Quantity)FROM PCH1 WHERE DocEntry=T6.DocEntry AND ItemCode=T6.ItemCode AND ObjType=T6.ObjType )+
...
OR (SELECT COUNT(DOCENTRY) FROM PCH1 WHERE BaseEntry=T6.PO_# and BaseType=22 )>1
THEN T6.Quantity - ISNULL((SELECT DISTINCT A1.QUANTITY FROM IGE21 A0
...
T0.PriceAfVAT AS 'Price After Discount',
T0.INMPrice AS 'Price After Discount(VAT-Ex)',
--ISNULL(ISNULL(T6.INMPrice,ISNULL(T7.INMPrice,T8.INMPrice)),T9.INMPrice)	AS 'COST', --T9.INMPrice
ISNULL(T6.INMPrice + ISNULL(((T0.INMPrice*(T0.Quantity - ISNULL(T11.Quantity,0))/T12.[SALES SUM])*T6.[LANDEDCOST])/T0.Quantity - ISNULL(T11.Quantity,0),0),
ISNULL(T7.INMPrice + ISNULL(((T0.INMPrice*(T0.Quantity - ISNULL(T11.Quantity,0))/T12.[SALES SUM])*T7.[LANDEDCOST])/T0.Quantity - ISNULL(T11.Quantity,0),0),
ISNULL(T8.INMPrice + ISNULL(((T0.INMPrice*(T0.Quantity - ISNULL(T11.Quantity,0))/T12.[SALES SUM])*T8.[LANDEDCOST])/T0.Quantity - ISNULL(T11.Quantity,0),0),
T9.INMPrice + ISNULL(((T0.INMPrice*(T0.Quantity - ISNULL(T11.Quantity,0))/T12.[SALES SUM])*T10.[LANDEDCOST])/T0.Quantity - ISNULL(T11.Quantity,0),0) 
))) AS 'COST',
...
LEFT JOIN (	SELECT DISTINCT A3.INMPrice,A3.DocEntry,A0.BaseEntry,A3.Quantity,A3.ItemCode,A3.WhsCode,A3.ObjType,A4.TAXDATE, 'AP'AS 'RTYPE',a0.DocEntry AS 'PO_#',
			...
			UNION ALL
			SELECT DISTINCT A3.INMPrice,A3.DocEntry,A0.BaseEntry,A3.Quantity,A3.ItemCode,A3.WhsCode,A3.ObjType,A4.TAXDATE, 'AP'AS 'RTYPE',a2.DocEntry AS 'PO_#',
			0 AS 'LANDEDCOST' FROM PRQ1 A0 
			...
			ON T6.BaseEntry=T0.BaseEntry
			AND T6.ItemCode=T0.ItemCode AND T6.WhsCode=T0.WhsCode


LEFT JOIN (SELECT DISTINCT A0.DOCENTRY,A4.RefObjType,A4.REFDOCNUM,A4.DOCENTRY AS 'REF_ENTRY',A4.ISSUEDATE,A0.INMPrice,A0.ItemCode,A0.ObjType,A0.Quantity,'AP'AS 'RTYPE',
			0 AS 'LANDEDCOST' FROM PCH1 A0
...
LEFT JOIN (SELECT DISTINCT A0.DOCENTRY,A4.RefObjType,A4.REFDOCNUM,A4.DOCENTRY AS 'REF_ENTRY',A4.ISSUEDATE,A0.INMPrice,A0.ItemCode,A0.ObjType,A0.Quantity,'AP'AS 'RTYPE',
		...
		AND A0.DOCENTRY NOT IN (SELECT BASEENTRY FROM PCH1 WHERE BaseType=22 AND ITEMCODE IS NOT NULL)) 
		AS T8
		ON T0.BaseEntry=T8.REF_ENTRY AND T8.ItemCode=T0.ItemCode
LEFT JOIN (SELECT DISTINCT A0.DOCENTRY,A4.RefObjType,A4.REFDOCNUM,A4.DOCENTRY AS 'REF_ENTRY',A5.TaxDate,A0.INMPrice,A0.ItemCode,A0.ObjType,A0.Quantity FROM PCH1 A0
		...
		INNER JOIN OPCH A5 ON A5.DOCNUM=A0.DOCENTRY
		WHERE A4.RefObjType=13 AND A5.CANCELED='N') AS T9
		ON T0.DocEntry=T9.REFDOCNUM AND T0.ItemCode=T9.ItemCode
--LANDED COST
