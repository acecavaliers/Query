...
SET @DATEFROM  ='2023-02-01'
SET @DATETO  ='2023-03-30'
SET @DEPARTMENT =''
SET @CATEGORY =''
SET @ITEMNAME =''
SET @STORE ='NAP'
...
IF (@AR='AR')
    ...
    T0.INMPrice AS 'Price After Discount(VAT-Ex)',
        ...
        ISNULL(CASE 
        WHEN T5.INMPrice IS NULL
        THEN 9999 --IIF(T6.NumPerMsr>T0.NumPerMsr,(T6.INMPrice/T6.NumPerMsr),T6.INMPrice) + ISNULL(((T0.INMPrice*(T0.Quantity - ISNULL(T11.Quantity,0))/T12.[SALES SUM])*T6.[LANDEDCOST])/T0.Quantity - ISNULL(T11.Quantity,0),0)    
        ELSE IIF(T5.NumPerMsr>T0.NumPerMsr,(T5.INMPrice/T5.NumPerMsr),T5.INMPrice) + ISNULL(((T0.INMPrice*(T0.Quantity - ISNULL(T11.Quantity,0))/T12.[SALES SUM])*T5.[LANDEDCOST])/T0.Quantity - ISNULL(T11.Quantity,0),0)
        END,9999--IIF(T7.NumPerMsr>T0.NumPerMsr,(T7.INMPrice/T7.NumPerMsr),T7.INMPrice) + ISNULL(((T0.INMPrice*(T0.Quantity - ISNULL(T11.Quantity,0))/T12.[SALES SUM])*T7.[LANDEDCOST])/T0.Quantity - ISNULL(T11.Quantity,0),0)
        )
         AS 'COST',
    ...
    LEFT JOIN (	
                ...
                ,TB.QUANTITY-ISNULL((SELECT SUM(SB.QUANTITY) AS APQTY FROM OPCH SA
                    INNER JOIN PCH1 SB ON SA.DocNum=SB.DocEntry
                    WHERE SA.CANCELED='N'
                    AND SB.BASEREF=TB.DocEntry AND SB.BaseType=TB.ObjType AND SB.ItemCode=TB.ItemCode),0) AS 'TTLAPQTY' 
                FROM INV21 TA
                INNER JOIN POR1 TB ON TA.RefDocNum=TB.DocEntry 
                INNER JOIN OPOR TC ON TB.DocEntry=TC.DocNum
                LEFT JOIN (SELECT AA.DocEntry,AA.ItemCode,AA.Quantity,AA.ObjType FROM PCH1 AA INNER JOIN OPCH AB ON AA.DocEntry=AB.DocNum WHERE AB.CANCELED='N' AND AA.BaseType=22)
                AS TD ON TD.ObjType=TB.BaseType
                -- INNER JOIN PCH1 TD ON TB.DOCENTRY=TD.BaseRef AND TB.ObjType=TD.BaseType
                -- LEFT JOIN OPCH  TD ON TD.DOCNUM=TB.BASEENTRY AND TD.ObjType=TB.BaseType AND TD.CANCELED='N'
    ...
    WHERE 
        D.[Quantity Sold]<>0
    AND 
        D.[BRANCH ID] NOT IN (SELECT BPLId FROM OBPL WHERE U_isDC='Y')
    AND 
        D.Department LIKE '%'+@DEPARTMENT+'%' 
    AND 
        D.Category LIKE '%'+@CATEGORY+'%'
    AND 
        D.Description LIKE '%'+@ITEMNAME+'%'
    AND 
        D.Whse LIKE '%'+@STORE+'%'
    AND 
        D.[Posting Date] BETWEEN @DATEFROM AND  @DATETO
    AND 
        D.CANCELED='N'
