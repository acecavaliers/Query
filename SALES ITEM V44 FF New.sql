...
END AS 'JE-COST',
--CASE WHEN D.TYPE='DELIVERY'
--	THEN (SELECT NUMBER FROM OJDT WHERE OJDT.BaseRef=D.DE AND OJDT.TransType =15) 
--	WHEN D.Reference LIKE '%AP%'
--	THEN (SELECT NUMBER FROM OJDT WHERE OJDT.BaseRef=D.[DE-AP] AND OJDT.TransType =18) 
--	ELSE 
--		CASE WHEN TYPE='AR RESERVE' AND (SELECT COUNT(BASEENTRY) FROM DLN1 WHERE BaseEntry=D.Transaction# AND DLN1.ItemCode=D.[Item Code])=0
--			THEN D.Transaction#
--			ELSE D.Number
--			END
--	 END AS Num,
...
INNER JOIN OINV T3 ON T0.DocEntry = T3.DocNum
INNER JOIN OCRD T4 ON T3.CardCode = T4.CardCode
LEFT JOIN (	SELECT DISTINCT A3.INMPrice,DocTotal AS 'LANDEDCOST' ,A3.DocEntry,A0.BaseEntry,A3.Quantity,A3.ItemCode,A3.WhsCode,A3.ObjType,A4.TAXDATE, 'AP'AS 'RTYPE',a0.DocEntry AS 'PO_#' FROM POR1 A0 
			...
			UNION ALL
			SELECT DISTINCT A2.INMPrice,U_LandedCost,A2.DocEntry,A2.BaseEntry,A2.Quantity,A2.ItemCode,A2.WhsCode,A2.ObjType,A3.TAXDATE,'PO'AS 'RTYPE',A2.DocEntry AS 'PO_#' FROM POR1 A2
			...
			UNION ALL
			SELECT DISTINCT A3.INMPrice,DocTotal AS 'LANDEDCOST' ,A3.DocEntry,A0.BaseEntry,A3.Quantity,A3.ItemCode,A3.WhsCode,A3.ObjType,A4.TAXDATE, 'AP'AS 'RTYPE',a2.DocEntry AS 'PO_#' FROM PRQ1 A0 
			...
			AND A3.ITEMCODE IS NOT NULL
			UNION ALL
			SELECT DISTINCT A2.INMPrice,U_LandedCost,A2.DocEntry,A0.BaseEntry,A2.Quantity,A2.ItemCode,A2.WhsCode,A2.ObjType,A3.TAXDATE,'PO'AS 'RTYPE',a2.DocEntry AS 'PO_#' FROM PRQ1 A0 
			INNER JOIN PQT1 A1 ON A0.DocEntry=A1.BaseEntry
			INNER JOIN POR1 A2 ON A1.DocEntry=A2.BaseEntry
			INNER JOIN OPOR A3 ON A2.DocEntry=A3.DocNum
			WHERE A2.BaseType=540000006  AND A3.CANCELED='N'
...
LEFT JOIN (SELECT DISTINCT A0.DOCENTRY,DocTotal AS 'LANDEDCOST' ,A4.RefObjType,A4.REFDOCNUM,A4.DOCENTRY AS 'REF_ENTRY',A4.ISSUEDATE,A0.INMPrice,A0.ItemCode,A0.ObjType,A0.Quantity,'AP'AS 'RTYPE' FROM PCH1 A0
		...
		UNION ALL
		SELECT DISTINCT A0.DOCENTRY,U_LandedCost,A3.RefObjType,A3.REFDOCNUM,A3.DOCENTRY AS 'REF_ENTRY',A3.ISSUEDATE,A0.INMPrice,A0.ItemCode,A0.ObjType,A0.Quantity,'PO'AS 'RTYPE' FROM POR1 A0
...
LEFT JOIN (SELECT DISTINCT A0.DOCENTRY,DocTotal AS 'LANDEDCOST' ,A4.RefObjType,A4.REFDOCNUM,A4.DOCENTRY AS 'REF_ENTRY',A5.TaxDate,A0.INMPrice,A0.ItemCode,A0.ObjType,A0.Quantity FROM PCH1 A0
...
SELECT T2.ItmsGrpNam AS 'Department',T3.CANCELED,T1.SWW,'' AS 'DE-AP','' AS 'TRANSTYPE',
T3.BPLID AS 'BRANCH ID',
CASE WHEN T3.isIns ='Y' THEN 'AR RESERVE GE' ELSE 'AR INVOICE GE' END AS 'TYPE',
T1.U_Category AS 'Category',
