...
-- @SortBy VARCHAR(100)='{?SortBy}'
 ...
 @STORE VARCHAR(10)='dcc',
 @SortBy VARCHAR(100)=''

SET @STORE=REPLACE(@STORE,'ALL','')
...
T0.Quantity*T0.NumPerMsr AS 'Quantity Sold',

T0.StockValue/(T0.Quantity *T0.NumPerMsr)  AS 'COST',

T0.INMPrice/T0.NumPerMsr AS 'TotalSales'
...
T0.UomCode2 AS 'unit',
T0.Dscription AS 'DESCRPTN',
(T0.Quantity-ISNULL(T7.QTY,0))* T0.NumPerMsr AS 'Quantity Sold',
...
T0.INMPrice/T0.NumPerMsr AS 'Total Sales'
...
T0.UomCode2 AS 'unit',
T0.Dscription AS 'DESCRPTN',
(T0.Quantity - ISNULL(T11.Quantity,0))*T0.NumPerMsr AS 'Quantity Sold',   
...
ISNULL(CASE 
	...
	-- CASE 
    -- WHEN T5.INMPrice IS NULL
	-- THEN (T6.Quantity *T6.NumPerMsr)
	-- ELSE (T5.Quantity *T5.NumPerMsr)
	-- END
	,(T0.StockValue/(T0.Quantity *T0.NumPerMsr))) 	
	AS 'COST',
...
INNER JOIN (SELECT A1.DOCENTRY,REFDOCNUM, A1.QUANTITY*A1.NumPerMsr AS QUANTITY,A1.LINETOTAL/(A1.QUANTITY*A1.NumPerMsr) AS 'PRICE',A0.IssueDate FROM IGE21 A0
...
T0.UomCode2 AS 'unit',
T0.Dscription AS 'DESCRPTN',
(T0.Quantity * T0.NumPerMsr) * -1 AS 'Quantity Sold',
ISNULL(
CASE 
...
END /(T0.NumPerMsr*-1) ,T0.StockValue/(T0.Quantity *T0.NumPerMsr) )
AS 'Cost',
(T0.INMPrice/T0.NumPerMsr) * -1 AS 'Total Sales'
...
LEFT JOIN (SELECT  A6.DocNum AS 'DocEntry',A0.INMPrice +ISNULL((((A7.INMPrice*(A7.Quantity - ISNULL(A8.Quantity,0))/A10.[SALES SUM])*A9.LANDEDCOST)/A7.Quantity - ISNULL(A8.Quantity,0)),0)AS 'INMPrice' ,A0.ItemCode
		...
		UNION ALL
		--FROM PO COST
		SELECT DISTINCT A2.INMPrice,A4.DocEntry,A2.ItemCode  
			FROM POR1 A2
