...
--  @DEPARTMENT VARCHAR(100)='{?Department}',@CATEGORY VARCHAR(100)='{?Category}',@ITEMNAME VARCHAR(200)='{?ItemName}',@STORE VARCHAR(10)='{?Whse}'
 DECLARE @DATEFROM DATE ='2022-10-20', @DATETO DATE ='2023-12-05',
...
-- --VIA AP
LEFT JOIN (
		SELECT TC.NumPerMsr, TT.RefDocNum,TD.DocNum,TD.TaxDate ,TC.Quantity,TC.ItemCode,TC.WhsCode,TC.INMPRICE
		...
		END AS 'LANDEDCOST'
        FROM PCH21 TT
        INNER JOIN PCH1 TC ON TT.DocEntry=TC.BaseEntry --AND TC.BaseType=22
